  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        console.error("Failed to load peer mentor data:", err);
      }
    };
    fetchAppointments();
  }, [currentUserId]);
    ...
    <div className="mentoring-layout">
      <main className="mentoring-main">
          ...
          {requests.map((req) => (
            <div key={req._id} className="booking-item" style={{ background: '#fff', border: '1px solid #ffeeba', borderLeft: '4px solid #ffc107' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <button onClick={() => handleUpdateStatus(req._id, 'Accepted')} style={{ background: '#28a745', color: 'white', padding: '8px 16px', borderRadius: '8px', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>Accept</button>
                <button onClick={() => handleUpdateStatus(req._id, 'Declined')} style={{ background: '#dc3545', color: 'white', padding: '8px 16px', borderRadius: '8px', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>Deny</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bookings-list">
          ...
          {sessions.length === 0 ? <p style={{ color: '#666', marginTop: '10px' }}>No upcoming sessions yet.</p> : null}
