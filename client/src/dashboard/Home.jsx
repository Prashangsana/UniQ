...
// Mentoring Imports (From your branch)
...
const Home = ({ myEventsList }) => {
  ...
  // Sync active tab with location state (important for navigation)
  useEffect(() => {
  ...
  // Handle direct URL paths
  useEffect(() => {
    const path = window.location.pathname;
    if (path.includes('/society') || path.includes('/events')) {
  ...
  const renderContent = () => {
    switch(activeTab) {
      ...
      case 'groups':    return <GroupsPage />; // Using imported GroupsPage from HEAD
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-profile">
          <div className="profile-img-container" onClick={() => setActiveTab('profile')} style={{cursor:'pointer'}}>
            <img src={sidebarUser.img} alt="User Profile" className="profile-img" />
        ...
        <nav className="sidebar-nav">
          <ul>
            ...
            {/* Mentoring Sidebar Link Integrated */}
